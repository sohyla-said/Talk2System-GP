...
export default function RequirementsChoicePage() {
	...
	useEffect(() => {
		if (location.state) {
			try {
				localStorage.setItem(STORAGE_KEY, JSON.stringify(location.state));
			} catch (e) {}
	...
	/**
	 * Resolves the correct transcript text to send to the extraction endpoint.
	 *
	 * Priority:
	 *   1. Cached translation from the backend (always checked first for non-English sessions).
	 *      This prevents using original-language text that may have been passed via nav state.
	 *   2. The transcriptText passed in via navigation state — only used as a fallback
	 *      when no translation exists (i.e. the session is English).
	 *   3. Original transcript segments fetched fresh from the API.
	 */
	const fetchTranscriptText = async () => {
		// ── Step 1: Always try the translation endpoint first ──────────────────
		// This ensures that even if the nav-state transcriptText was the original
		// non-English text (e.g. translation wasn't ready when extraction was first
		// triggered), the regeneration still uses the correct English translation.
		try {
			...
			// Translation fetch failed — fall through to nav state / original below
		}

		// ── Step 2: Use nav-state transcriptText if present ────────────────────
		// At this point we know either (a) the translation endpoint returned nothing
		// (session is English) or (b) the fetch failed. In both cases the nav-state
		// text is safe to use if available.
		if (transcriptText && transcriptText.trim()) {
			return transcriptText;
		}

		// ── Step 3: Fetch original transcript segments as a last resort ────────
	...
	const handlePrefer = async (type) => {
		...
		try {
			...
			// Clear persisted extraction state — user has chosen a preferred set
			try {
...
function ResultPane({ title, subtitle, grouped, diffGrouped, buttonLabel, isLoading, onPrefer }) {
