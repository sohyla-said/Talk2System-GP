...
//   const [sessionTitle] = useState(location.state?.sessionTitle || '');
...
const TranscriptInputPage = () => {
  ...
  const location = useLocation();
  const [sessionTitle] = useState(location.state?.sessionTitle || '');
  const participants = location.state?.participants || []; // non-PM user IDs; backend resolves and adds PM automatically
  useEffect(() => {
    setCharCount(transcript.length);
  }, [transcript]);
