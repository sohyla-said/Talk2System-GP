...
export default function AdminAddProjectPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createProject({ name, description, domain, manager_email: managerEmail });
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display">
      <div className="max-w-[960px] mx-auto px-4 sm:px-8 py-6">
        ...
        <div className="mb-6">
          <button
            onClick={() => navigate("/projects/system-projects")}
          ...
          <div className="p-6 md:p-8">
            <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
              ...
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Talk2System Mobile App" required
              ...
              <div>
                ...
                <textarea rows={5} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the project scope..."
              ...
              <div>
                ...
                <input type="text" value={domain} onChange={(e) => setDomain(e.target.value)} placeholder="e.g. Healthcare, Fintech, Education"
                  className="w-full rounded-lg bg-background-light dark:bg-background-dark/80 px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 transition-all" />
              </div>
              ...
              <div className="p-4 border-2 border-dashed border-primary/30 rounded-lg bg-primary/5">
                <label className="block text-sm font-bold mb-1 text-primary">
                  Assign Project Manager (User Email)
                </label>
                <p className="text-xs text-gray-500 mb-2">Type the registered user's email who will manage this project.</p>
                <input 
                  type="email" 
                  value={managerEmail} 
                  onChange={(e) => setManagerEmail(e.target.value)} 
                  placeholder="e.g. user@example.com"
                  className="w-full rounded-lg bg-white dark:bg-background-dark px-4 py-3 text-sm focus:ring-2 focus:ring-primary/20 transition-all" 
                />
              </div>

              <div className="flex gap-4 pt-4">
                <button type="submit" disabled={loading}
                  className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-white font-bold shadow-lg transition-all ${loading ? "bg-gray-400 cursor-not-allowed" : "bg-primary hover:bg-primary/90 shadow-primary/25"}`}>
                  <span className="material-symbols-outlined">save</span>
                  {loading ? "Creating..." : "Create Project"}
                </button>
                <button type="button" onClick={() => navigate("/projects/system-projects")}
